...
void ntt(vector<ll>&a,int f=0){
    int k=0,len=a.size();while((1<<k)<len)k++;
    for(int i=0;i<len;i++){
        int t=0;
        for(int j=0;j<k;j++){
            if(i>>j&1)t+=(1<<k-j-1);
...
struct poly{
    ...
    friend poly operator *(poly a,poly b){
        ...
        ntt(a.a),ntt(b.a);for(int i=0;i<a.a.size();i++)a.a[i]=a.a[i]*b.a[i]%mod;
        ntt(a.a,1);return a;
    }
};
...
signed main(){
    ...
    cin>>n;for(int i=1;i<=n;i++)cin>>d[i];init(n);
    ...
    poly f=solve(1,n);ll mul=1;
    res=fac[n-1];
    for(int i=1;i<n;i++){
        add(res,f.a[i]*(n-i)%mod*mul%mod*fac[n-i-1]);
        mul=mul*n%mod;
    }
    for(int i=1;i<=n;i++)res=res*inv[d[i]]%mod;
